...
vi.mock('../composables/useEnumOptions', () => ({
  useEnumOptions: () => ({
    relationshipStatusOptions: () => [{ value: 'single', label: 'Single' }],
  }),
}))
...
describe('RelationstatusSelector', () => {
  it('updates model on select', async () => {
    const wrapper = mount(RelationstatusSelector, {
      global: { stubs: { BListGroup: true, BListGroupItem: true, BFormRadio: true } },
    })
