...
describe('renderEmail', () => {
  ...
  it('removes <style> tags after inlining non-media-query rules', async () => {
    ...
    const nonMediaStyleTags = allStyleTags.filter(
      (tag) => !tag.includes('@media')
    )
  ...
  it('applies body inline styles to the <body> element', async () => {
    const result = await renderEmail(SimpleComponent, defaultProps)
    // The <body> tag has an explicit inline style set by renderEmail
    expect(result).toMatch(/<body[^>]*style="[^"]*margin:\s*0[^"]*"/)
  })
